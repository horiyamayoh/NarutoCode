...
- 厳密モードは `-StrictMode` スイッチで有効化する。
  従来の動作（概算モード）はデフォルトとして維持する。

### 1.2 パラメータ設計

```powershell
# 新規追加パラメータ
[switch]$StrictMode    # 全指標の厳密計算を有効化
```

`-StrictMode` を指定した場合：
- `DeadDetailLevel` は自動的に最大（2）として扱う
- per-revision blame を実行する（`-NoBlame` と排他）
- `Get-RoundedNumber` による丸めを行わない
- `max(1, x)` によるゼロ除算回避を行わず、`$null` を出力する
- `max(0, dead)` クランプを行わない（per-revision blame により負値は原理的に発生しない）
- 共変更分析の 100 ファイル上限を撤廃する
- リネーム時の追加/削除行数の二重計上を補正する
- ハッシュベースの指標を per-revision blame ベースに置き換える
...
|-----|--------|--------|----------|----------------|
| 1 | 消滅追加行数 (概算) | committers / files | A | G1: per-revision blame |
| 2 | 自己消滅行数 (概算) | committers | A | G1: per-revision blame |
| 3 | 被他者消滅行数 (概算) | committers | A | G1: per-revision blame |
| 4 | 自己相殺行数 | committers | B | G1: per-revision blame |
| 5 | 自己差戻行数 | committers | B | G1: per-revision blame |
| 6 | 他者差戻行数 | committers | B | G1: per-revision blame |
| 7 | 被他者削除行数 | committers | B | G1: per-revision blame |
...
| No. | 指標名 | 現行の近似原因 |
|-----|--------|---------------|
| 1 | 消滅追加行数 (概算) | `追加行数 − 生存行数` — diff 累積値と blame スナップショットの不整合 |
| 2 | 自己消滅行数 (概算) | `min(自己相殺行数, 消滅追加行数)` — ハッシュベースの自己相殺を上限として使用 |
| 3 | 被他者消滅行数 (概算) | `max(0, 消滅 − 自己消滅)` — 残差計算 |
| 4 | 自己相殺行数 | SHA1 ハッシュ照合 — トリビアル行（`}` 等）で偽陽性 |
| 5 | 自己差戻行数 | No.4 のエイリアス |
| 6 | 他者差戻行数 | 同じハッシュ照合問題 |
| 7 | 被他者削除行数 | 同じハッシュ照合問題 |
...
| No. | 指標名 | 現行の近似原因 |
|-----|--------|---------------|
| 18 | 共変更回数 | 100 ファイル超のコミットを除外 |
| 19 | Jaccard | 同上 + 丸め |
| 20 | リフト値 | 同上 + 丸め |

### 7.2 現行アルゴリズムの問題点

`Get-CoChangeMetric`（L1195-L1290）のパラメータ `$LargeCommitFileThreshold = 100` により、
100 ファイル以上を含むコミットは co-change 集計からスキップされる（L1219-L1221）。
...
これにより大規模マージやリファクタリングコミットでの共変更関係が失われる。
