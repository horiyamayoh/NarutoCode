...
| **消滅追加行数 (概算)** | `追加行数 − 生存行数`（消えた行数の近似） | **手戻り量の推定**。大きいと書いたコードの多くが最終的に不要になった |
...
| **消滅追加行数 (概算)** | `追加行数 − 生存行数`（消えた行数の近似） | 大きいとこのファイルで手戻りが多い |

### 3.5 ホットスポット

| 列名 | 計算式 | 読み方 |
|---|---|---|
| **ホットスポットスコア** | `コミット数 × 総チャーン` | **「頻繁に・大量に変わるファイル」** ほどスコアが高い。外部の複雑度指標なしに「問題のありそうなファイル」を特定するための指標 |
...
| 見たい観点 | 組み合わせ | 判断基準 |
|---|---|---|
| 手戻りが多い開発者 | `消滅追加行数 (概算)` ÷ `追加行数` | 比率が高いほど書いたコードが残っていない |
| やり直しの傾向 | `チャーン対純増比` が高い + `消滅追加行数 (概算)` が高い | 両方高いと手戻りが頻発している |
